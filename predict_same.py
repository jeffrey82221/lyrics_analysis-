...
#################################################################################
filename = 'win6.w100.l32.d128.embeddings'
import numpy as np
result_lines = [line.rstrip('\n') for line in open(filename)]
object_count = len(result_lines)
splited_result_lines = []
...
voc_dict=pickle.load(open('dict.voc','rb'))
voc_size = len(voc_dict[0])
song_lyrics_em2 = embedding_all[voc_size:,:]
song_keys = embedding_key[voc_size:]
...
output_data = song_lyrics_em_matched2
...
for i in range(np.shape(input_data)[1]):
    ...
    output_data[:,i]=(song_lyrics_em_matched2[:,i]-np.mean(song_lyrics_em_matched2[:,i]))/(np.var(song_lyrics_em_matched2[:,i])**0.5)
